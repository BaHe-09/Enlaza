...
model = load_model("modelo_reconocimiento_senas_1_tipo1_28.h5")

# Leer las etiquetas desde la columna "label" del archivo CSV
data = pd.read_csv("puntos_clave.csv")
class_names = data['label'].unique().tolist()  # Extraer las etiquetas únicas como una lista
print(class_names)
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            keypoints = np.array(keypoints).reshape(1, -1)  # Convertir a formato de entrada del modelo matriz/tensor
                                                            # Redimensión automatica
